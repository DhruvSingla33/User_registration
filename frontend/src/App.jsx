...
const App = () => {
  ...
  const isE164 = (val) => /^\+[1-9]\d{1,14}$/.test(val);

  function normalizeToE164(nationalOrFull, dialCode) {
    const raw = String(nationalOrFull || "").trim();
    if (!raw) return null;

    if (raw.startsWith("+")) {
      return isE164(raw) ? raw : null;
    }
    ...
    const prefix = dialCode && dialCode.startsWith("+") ? dialCode : `+${String(dialCode || "").replace(/\D/g, "")}`;
    const candidate = `${prefix}${digits}`;
    return isE164(candidate) ? candidate : null;
  }

  const checkUser = async (phoneE164) => {
    try {
      const res = await fetch(`${API}/api/check-user?phone=${encodeURIComponent(phoneE164)}`);
  ...
  const sendOtp = async () => {
    ...
    const e164 = normalizeToE164(rawPhoneInput, countryCode);
    if (!e164) {
      setMsgType("error");
      setMsg("Please enter a valid phone number (either full +countrynumber or select country code and type number).");
      return;
    }
    ...
    const check = await checkUser(e164);
    if (check.success && check.exists && check.verified) {
      setUserData(check.user);
      setMsgType("success");
      setMsg("This phone is already verified and registered ✅");
      setStep("done");
      setLoading(false);
      return;
    }
    ...
    try {
      const res = await fetch(`${API}/api/send-otp-sms`, {
        method: "POST",
      ...
      if (body.success) {
        ...
        setRawPhoneInput(""); // clear input if you like
      } else {
        setMsgType("error");
  ...
  const verifyOtp = async () => {
    ...
    try {
      const resp = await fetch(`${API}/api/verify-otp-sms`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (resp.ok && data.verified) {
        setMsgType("success");
        setMsg("Phone verified ✅ Fill your details to complete registration.");
        setStep("register");
      } else {
        setMsgType("error");
  ...
  const register = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API}/api/register-sms`, {
        phone,
        firstName,
        lastName,
        address,
    ...
    <div style={styles.page}>
      <div style={styles.card}>
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              ...
              <input
                ...
                type="tel"
                placeholder="Enter phone number (national)"
                value={rawPhoneInput}
        ...
        {step === "otp" && (
          <>
            <p>
              We sent a 6-digit code to <b>{phone || `${countryCode} ${rawPhoneInput}`}</b>
            </p>
            ...
            <div style={{ marginTop: 12 }}>
              <button style={styles.link} onClick={goBackToPhone} disabled={loading}>
        ...
        {step === "done" && (
          <>
            ...
            <p>
              Phone: <b>{userData?.phone || phone}</b>
            </p>
            {userData?.address && (
              <p>
